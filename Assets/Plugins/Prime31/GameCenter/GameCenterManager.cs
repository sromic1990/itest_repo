...
namespace Prime31
{
	public class GameCenterManager : AbstractManager
	{
		...
		// Fired when a challenge is successfully sent. Includes an array of all the playerIds that the challenge was sent to.
		public static event Action<List<object>> challengeIssuedSuccessfullyEvent;

		// Fired when the challenge composer completes and the user did not send the challenge to anyone.
		public static event Action challengeNotIssuedEvent;

		#endregion


		#region Game Save Events

		// Fired when a call to fetchSavedGames succeeds
		public static event Action<List<GKSavedGame>> fetchSavedGamesSucceededEvent;
		
		// Fired when a call to fetchSavedGames fails
		public static event Action<P31Error> fetchSavedGamesFailedEvent;
		
		// Fired when a call to saveGame succeeds
		public static event Action saveGameSucceededEvent;
		
		// Fired when a call to saveGame fails
		public static event Action<P31Error> saveGameFailedEvent;
		
		// Fired when deleteSavedGame succeeds
		public static event Action deleteSavedGameSucceededEvent;
		
		// Fired when deleteSavedGame fails
		public static event Action<P31Error> deleteSavedGameFailedEvent;
		
		// Fired when a call to loadSavedGameData succeeds
		public static event Action<byte[]> loadSavedGameDataSucceededEvent;
		
		// Fired when a call to loadSavedGameData fails
		public static event Action<P31Error> loadSavedGameDataFailedEvent;

		#endregion


		delegate void GKSavedGameDataLoadedCallback( IntPtr dataBuf, int length );

		[DllImport("__Internal")]
		static extern void _gameCenterSetSavedGameDataLoadedCallback( GKSavedGameDataLoadedCallback callback );
	    ...
	    static GameCenterManager()
	    {
			...
			if( Application.platform == RuntimePlatform.IPhonePlayer )
				_gameCenterSetSavedGameDataLoadedCallback( savedGameDataLoaded );
	    }


		[AOT.MonoPInvokeCallback( typeof( GKSavedGameDataLoadedCallback ) )]
		private static void savedGameDataLoaded( IntPtr dataBuf, int length )
		{
			var data = new byte[length];
			Marshal.Copy( dataBuf, data, 0, length );
	
			if( loadSavedGameDataSucceededEvent != null )
				loadSavedGameDataSucceededEvent( data );
		}
		...
		void fetchSavedGamesSucceeded( string json )
		{
			if( fetchSavedGamesSucceededEvent != null )
				fetchSavedGamesSucceededEvent( Json.decode<List<GKSavedGame>>( json ) );
		}
		...
		void saveGameSucceeded( string empty )
		{
			if( saveGameSucceededEvent != null )
				saveGameSucceededEvent();
		}
		...
		void deleteSavedGameSucceeded( string empty )
		{
			if( deleteSavedGameSucceededEvent != null )
				deleteSavedGameSucceededEvent();
		}
