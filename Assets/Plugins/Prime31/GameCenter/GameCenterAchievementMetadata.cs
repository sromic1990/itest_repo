...
namespace Prime31
{
	public class GameCenterAchievementMetadata
	{
		...
		public GameCenterAchievementMetadata( Dictionary<string,object> dict )
		{
			...
			if( dict.ContainsKey( "isReplayable" ) )
				isReplayable = (bool)dict["isReplayable"];
		}
	
	
		public override string ToString()
		{
		 	return string.Format( "<AchievementMetaData> identifier: {0}, hidden: {1}, maxPoints: {2}, title: {3} desc: {4}, unachDesc: {5}, isReplayable: {6}",
		 		identifier, isHidden, maximumPoints, title, description, unachievedDescription, isReplayable );
		}
